...
describe('aliasPlugin', () => {
  ...
  it('resolve alias config', () => {
    ...
    expect(
      resolveAliasConfig(
        ...
        '/workspace',
        '/workspace/src/components/Button/index.tsx',
      ),
    ).toEqual({
      '@': '../..',
    });
  ...
  it('alias plugin transform', async () => {
    const plugin = aliasPlugin('/workspace', {
      '@': './src',
      react: 'react-dom',
    });
    const result1 = await (plugin as any).transform('console.log()', '/workspace/src/index.tsx');
    expect(result1.code).toBe('console.log()');

    const result2 = await (plugin as any).transform('import react from "react";', '/workspace/src/index.tsx');
