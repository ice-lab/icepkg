...
import {
  BuildTask,
  BundleTaskConfig, BundleUserConfig,
  DeclarationTaskConfig, StandardBundleFormatString, BundleFormat,
...
// Transform Task
function tt(format: string, standardFormat: StandardTransformFormatString, config: Omit<TransformTaskConfig, 'type' | 'format' | 'originalFormat'>): BuildTask {
  return {
...
function bt(format: string, standardFormat: StandardBundleFormatString[], config: Partial<BundleTaskConfig>): _BuildTask<BundleTaskConfig> {
  return {
    name: `bundle-${format}`,
    config: {
      type: 'bundle',
...
describe('initTask', () => {
  describe('entry', () => {
    ...
    it('with entry', () => {
      const task = initTask(tt('esm', 'esm:es5', {
          entry: 'src/index2'
        }), c({          entry: 'src/index'
        })
      )
  ...
  describe('sourcemap', () => {
    ...
    it('both no sourcemap', () => {
      for (const [newCommand, expected] of [['build', false], ['start', true]] as [string, boolean][]) {
        command = newCommand
        const task = initTask(tt('esm', 'esm:es5', {}), c({
        }))
  ...
  describe('transform', () => {
    describe('modes', () => {
      it('default is based on command', () => {
        for (const [newCommand, expected] of [['build', 'production'], ['start', 'development'], ['test', 'development']] as [string, string][]) {
          command = newCommand
          const task = initTask(tt('esm', 'esm:es5', {}), c({}))
  ...
  describe('bundle', () => {
    describe('modes', () => {
      it('default is based on command', () => {
        for (const [newCommand, expected] of [['build', ['production']], ['start', ['development']]] as [string, string[]][]) {
          command = newCommand
    ...
    describe('outputDir', () => {
      ...
      it('taskConfig is preset', () => {
        const task = initTask(bt('esm', ['esm:es5'], {
          outputDir: 'custom1'
        }), c({
          bundle: {
            outputDir: 'custom'
    ...
    describe('minify', () => {
      ...
      it('boolean', () => {
        for (const value of [true, false]) {
          ...
          expect(minifyFunctionTable((task.config as BundleTaskConfig).cssMinify)).toEqual([value, value, value, value])
        }
      })
      ...
      it('object as boolean', () => {
        for (const jsValue of [undefined, true, false]) {
          for (const cssValue of [undefined, true, false]) {
            ...
            expect(minifyFunctionTable((task.config as BundleTaskConfig).jsMinify)).toEqual(jsValue === undefined ? DEFAULT_TABLE : [jsValue, jsValue, jsValue, jsValue])
            expect(minifyFunctionTable((task.config as BundleTaskConfig).cssMinify)).toEqual(cssValue === undefined ? DEFAULT_TABLE : [cssValue, cssValue, cssValue, cssValue])
          }
        }
      })
    });
      ...
      it(`taskConfig is preset`, () => {
        for (const taskConfigValue of validValues) {
          for (const userConfigValue of validValues) {
            ...
            expect((task.config as BundleTaskConfig)[key])
              .toEqual(taskConfigValue)
          }
        }
      })
    })
  ...
  describe('declaration', () => {
    ...
    describe('outputMode', () => {
      it('userConfig is true', () => {
        const task = initTask(dt(FORMATS, {}), c({
          declaration: true,
        }))
      ...
      it('taskConfig is present', () => {
        const task = initTask(dt(FORMATS, {
          outputMode: 'unique'
        }), c({
          declaration: true,
        }))
